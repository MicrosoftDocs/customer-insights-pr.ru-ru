...
---
# <a name="customer-lifetime-value-clv-prediction-preview"></a>Прогноз ценности клиентов на протяжении жизненного цикла (CLV) (предварительная версия)
...
- Данные о действиях клиентов (необязательно):
    ...
    - Схема семантических данных для действий включает: 
        ...
        - Метка времени: дата и время события, определенные первичным ключом
...
1. В области **Ценность клиента на протяжении жизненного цикла (предварительная версия)** выберите **Начать**.
...
1. Установить **Период времени прогноза**, чтобы определить, как далеко в будущем вы хотите прогнозировать CLV.    
   ...
   > [!TIP]
   > Чтобы точно предсказать CLV для установленного периода времени, вам потребуется сопоставимый период исторических данных. Например, если вы хотите сделать прогноз на следующие 12 месяцев, рекомендуется располагать историческими данными как минимум за 18–24 месяца.
...
Прогнозы автоматически обновляются по тому же [расписанию, по которому обновляются ваши данные](system.md#schedule-tab), как задано в настройках. Вы также можете обновить их вручную.

1. Перейдите **Аналитика** > **Прогнозы** и выберите вкладку **Мои прогнозы**.
2. Выберите вертикальное многоточие рядом с прогнозом, который вы хотите обновить.
3. Выбрать **обновить**.
...
1. Перейдите **Аналитика** > **Прогнозы** и выберите вкладку **Мои прогнозы**.
2. Выберите вертикальное многоточие рядом с прогнозом, который вы хотите удалить.
3. Выберите **Удалить**.

## <a name="troubleshoot-a-failed-prediction"></a>Устранение неполадок со сбоем прогноза

1. Перейдите **Аналитика** > **Прогнозы** и выберите вкладку **Мои прогнозы**.
2. Выберите вертикальные многоточия рядом с прогнозом, для которого вы хотите просмотреть журналы ошибок.
3. Выберите **Журналы**.
4. Просмотр всех ошибок. Существует несколько типов ошибок, которые описывают, в каком состоянии возникла ошибка. Например, ошибка, при которой недостаточно данных для точного прогнозирования, обычно устраняется путем загрузки дополнительных данных в аналитику аудитории.
