...
1. Перейдите **Данные** > **Объединить** > **Соответствие** и выберите **Задать порядок**, чтобы начать фазу соответствия.
1. Выберите **Порядок сущности**. Например, выберите **eCommerce:eCommerceContacts** как основную сущность и **LoyaltyScheme:loyCustomers** в качестве второй сущности. 
1. Чтобы каждая запись в сущности была уникальным клиентом и сопоставлялась с каждой последующей сущностью, выберите **Включить все**.
1. Нажмите кнопку **Готово**. 

После указания порядка сопоставления определенные пары совпадений отображаются в разделе **Сведения о сопоставленных записях** в **Данные** > **Унификация** > **Сопоставление**. Ключевые показатели остаются пустыми, пока не завершится процесс сопоставления.
...
Основная сущность *eCommerce:eCommerceContacts* сопоставлена со следующей сущностью *LoyaltyScheme:loyCustomers*. Набор данных, полученный в результате первого шага сопоставления, сопоставляется со следующей сущностью, если у вас более двух сущностей.
...
1. Выберите **Добавить правило** для сущности в разделе **Сведения о сопоставленных записях** для определения правил сопоставления.
...
1. В области **Создать правило** настройте условия для правила.
   ...
   - **Точность**: установите уровень точности, чтобы иметь доступ к этому условию. 
     - **Базовый**: выберите *Низкий*, *Средний*, *Высокий*, и *Точный*. Выберите **Точно**, чтобы сопоставлять только записи, которые совпадают на 100 процентов. Выберите один из других уровней, чтобы сопоставить записи, которые не идентичны на 100 процентов.
...
Вы можете изменить порядок сущностей для правил соответствия, чтобы изменить порядок их обработки. Правила, конфликтующие из-за изменения порядка, будут удалены. Вам необходимо воссоздать удаленные правила с обновленной конфигурацией.
...
Дедуплицированные записи используются в процессе сопоставления между сущностями. Дедупликация происходит для отдельных сущностей и может быть настроена для каждой сущности, используемой в парах соответствия.
...
1. В разделе **Сведения о дедуплицированных записях** выберите **Задать сущности**. Если правила дедупликации уже созданы, выберите **Редактировать**.
...
1. После [запуска процесса сопоставления](#run-the-match-process) вы увидите статистику дедупликации на плитках ключевых показателей.
...
## <a name="include-enriched-entities-preview"></a>Включить обогащенные сущности (предварительная версия)

Если вы обогатили сущности на уровне источника данных, выберите их перед запуском процесса сопоставления. Обогащенные сущности могут улучшить результаты унификации. Дополнительная информация дана в теме [Обогащение источников данных](data-sources-enrichment.md). 

Обогащенная сущность содержит исходные поля источника данных и обогащенные поля. Поэтому, если вы решите работать с обогащенной сущностью, существующая конфигурация не пострадает. Однако вам может потребоваться обновить правила сопоставления, чтобы использовать вместо них обогащенные поля.

1. Перейдите в **Данные** > **Унификация** > **Сопоставление** и выберите **Использовать обогащенные сущности** в верхней части страницы.

1. В области **Использовать обогащенные сущности** выберите одну или несколько обогащенных сущностей.

1. Нажмите кнопку **Готово**. Везде, где используется исходная сущность (например, порядок сопоставления или правила), она автоматически заменяется на обогащенную сущность.
...
1. В области **Изменить правило** выберите **Добавить исключение**.
...
1. В области **Пользовательское** перейдите на вкладку **Записи**.

1. Выберите вариант персонализированного соответствия из раскрывающегося списка **Пользовательский тип** и выберите **Загрузить шаблон**. Для каждого варианта совпадения нужен отдельный шаблон.
...
1. После того как отправленные файлы и сущностей доступны, выберите параметр **Пользовательское соответствие** снова. Вы увидите опции для указания сущностей, которые вы хотите включить. Выберите необходимые объекты из раскрывающегося меню и выберите **Готово**.
...
1. Применение настраиваемого сопоставления зависит от варианта сопоставления, который вы хотите использовать. 
   ...
   - Для **Пользовательский пропуск** или **Сопоставление псевдонима** выберите **Изменить** в существующем правиле соответствия или создайте новое правило. В раскрывающемся списке Нормализация выберите **Пользовательский пропуск** или **Сопоставление псевдонима** и выберите **Готово**.
...
- Самостоятельное объединение не показывает нормализованные данные в объектах дедупликации. Однако это применяет внутреннюю нормализацию во время дедупликации. Это сделано специально для всех нормализаций. 
- Если параметр семантического типа удален на фазе **Сопоставление**, когда правило соответствия использует сопоставление псевдонимов или пользовательский пропуск, нормализация не применяется. Это происходит только в том случае, если вы очищаете семантический тип после настройки нормализации в правиле соответствия, потому что семантический тип будет неизвестен.


## <a name="next-step"></a>Следующий шаг

После завершения процесса сопоставления хотя бы для одной пары совпадений переходите к этапу [**Объединение**](merge-entities.md).
