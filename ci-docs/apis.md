...
1. Чтобы включить API-интерфейсы в среде Customer Insights, перейдите в раздел **Администрирование** > **Безопасность**. Для этого вам потребуются права администратора.

1. Перейдите на вкладку **API** и выберите кнопку **Включить**.    
 
   Включение API создает первичный и вторичный ключ подписки для вашего экземпляра, который используется в запросах API. Можно повторно создать ключи, выбрав **Повторно создать первичный** или **Повторно создать вторичный** в разделе **Администрирование** > **Безопасность** > **API-интерфейсы**.
...
1. В заключение, мы должны добавить имя приложения для регистрации в качестве пользователя в Customer Insights.  
   ...
   Откройте Customer Insights, перейдите в раздел **Администрирование** > **Безопасность** и выберите пункт **Добавить пользователя**.
