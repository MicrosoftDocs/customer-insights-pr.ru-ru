---
title: Экспорт журналов диагностики (предварительная версия)
...
Пересылка журналов из Customer Insights с помощью Azure Monitor. Журналы ресурсов Azure Monitor позволяют вести мониторинг и отправлять журналы в [службу хранилища Azure](https://azure.microsoft.com/services/storage/),[Azure Log Analytics](/azure/azure-monitor/logs/log-analytics-overview), а также передавать их в потоковом режиме в [Центры событий Azure](https://azure.microsoft.com/services/event-hubs/).

Customer Insights отправляет следующие журналы событий:

- **События аудита**
  - **APIEvent** — позволяет отслеживать изменения через пользовательский интерфейс Dynamics 365 Customer Insights.
- **Операционные события**
  - **WorkflowEvent** — позволяет настроить [источники данных](data-sources.md), [объединить](data-unification.md), [обогатить](enrichment-hub.md) и [экспортировать](export-destinations.md) данные в другие системы. Эти шаги можно выполнить по отдельности (например, запустить один экспорт). Их также можно выполнить организованно (например, обновление данных из источников данных, которое запускает процесс объединения, который будет получать обогащения и экспортировать данные в другую систему). Дополнительные сведения см. в разделе [Схема WorkflowEvent](#workflow-event-schema).
  - **APIEvent** отправляет все вызовы API экземпляра клиента для Dynamics 365 Customer Insights. Дополнительные сведения см. в разделе [Схема APIEvent](#api-event-schema).
...
1. В Customer Insights выберите **Администратор** > **Система** и выберите вкладку **Диагностика**.

1. Выберите **Добавить пункт назначения**.

   :::image type="content" source="media/diagnostics-pane.png" alt-text="Подключение диагностики.":::
...
1. Выберите **Подписка**, **Группа ресурсов**, а также **Ресурс** для целевого ресурса. Сведения о разрешении и журнале см. в разделе [Конфигурация целевого ресурса](#configuration-on-the-destination-resource).
...
В зависимости от выбранного типа ресурса автоматически производятся следующие изменения:
...
Субъект-служба Customer Insights получает разрешение **Участник Log Analytics** в отношении ресурса. Журналы доступны в разделе **Журналы** > **Таблицы** > **Управление журналами** в выбранной рабочей области Log Analytics. Разверните решение **Управление журналами** и найдите таблицы `CIEventsAudit` и `CIEventsOperational`.
...
## <a name="remove-a-diagnostics-destination"></a>Удаление назначения диагностики

1. Перейдите в **Администрирование** > **Система** и выберите вкладку **Диагностика**.

1. Выберите пункт назначения диагностики в списке.

   > [!TIP]
   > Удаление назначения останавливает пересылку журнала, но не удаляет ресурс в подписке Azure. Для удаления ресурса в Azure выберите ссылку в столбце **Действия**, чтобы открыть портал Azure для выбранного ресурса и удалить его там. Затем удалите пункт назначения диагностики.

1. В столбце **Действия** выберите значок **Удалить**.

1. Подтвердите удаление, чтобы удалить назначение и остановить пересылку журнала.
...
События API и события рабочего процесса имеют общую структуру, но с некоторыми отличиями. Дополнительные сведения см. в разделе [Схема события API](#api-event-schema) или [Схема события рабочего процесса](#workflow-event-schema).
...
Рабочий процесс состоит из нескольких шагов: [прием источников данных](data-sources.md), [объединение](data-unification.md), [обогащение](enrichment-hub.md) и [экспорт](export-destinations.md) данных. Все эти шаги могут выполняться по отдельности или оркестрированно в сочетании со следующими процессами.

#### <a name="operation-types"></a>Типы операций

| Тип операции     | Групповой                                     |
| ----------------- | ----------------------------------------- |
