...
- Ваша среда должна быть настроена для **индивидуальных клиентов** в качестве основной целевой аудитории.
...
1. На шаге **Фильтры продуктов** выберите один из следующих вариантов.
...
1. На странице результатов есть пять основных разделов данных:
    ...
    Ниже приведены факторы, которые могут повлиять на рекомендацию продукта, выработанную моделью.
        - **Прошлые транзакции**: шаблоны покупок в прошлом использовались моделью для выработки рекомендаций по продуктам. Например, модель может порекомендовать *Мышь Surface Arc Mouse*, если кто-то недавно купил *Surface Book 3* и *Surface Pen*. Модель узнала, что исторически многие клиенты покупали мышь *Surface Arc Mouse* после покупки *Surface Book 3* и *Surface Pen*.
